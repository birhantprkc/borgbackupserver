...
class ProfileController extends Controller
{
    ...
    public function update(): void
    {
        ...
        // Low-storage alerts: a mute switch, nothing more. The threshold moved
        // to a single server-wide setting so the alert and /api/v1/health can't
        // disagree (#156 gave every user their own number, which is what made
        // them disagree). storage_alert_mode is reused as the on/off flag.
        if ($tab === 'storage_alerts') {
            $this->db->update('users', [
                'storage_alert_mode' => empty($_POST['storage_alert_enabled']) ? 'disabled' : 'percent',
            ], 'id = ?', [$userId]);
